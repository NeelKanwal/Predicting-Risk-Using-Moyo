...
# Training CVAE model in semi-supervised way
import warnings
...
from myfunctions import FHRDataset_v2, collate_fn, FocalLoss,visualize_reconstruction, calculate_fid, plot_reconstructions, \
 convert_batch_list, make_pretty_cm,  get_class_weights, weights_to_tensor, CVAE, cvae_loss, plot_random_signals, FHRDataset_encoder
...
train_dataset = FHRDataset_encoder(train_dir, sequence_length=input_dim, normalization=Norm, force_norm=True) #
    ...
    for sch in lr_scheduler:
        for lr in learning_rate:
            for i in range(iterations):
                ...
                print("Load Convolutional Autoencoder Architecture")
                ...
                for epoch in range(num_epochs):
                    ...
                    for data, labels in train_loader:
                        ...
                        loss = cvae_loss(recon_batch, data, mu, logvar, beta)
                        loss.backward()
                ...
                plt.savefig(f"{path}/LossCurve_CVAE.png")
                # plt.show()
                ...
                plot_reconstructions(real_features, generated_features, file_names,Norm, path, n=2)
