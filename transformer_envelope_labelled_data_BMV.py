...
# This is transformer-based classification model for BMV classification training. It uses new version of dataset and loads a different version of dataloader
# this uses two inputs (FHR) and evelope for training. Multivariate training
...
from myfunctions import  collate_fn_twoInputs, convert_batch_list, make_pretty_cm, get_class_weights, weights_to_tensor, plot_random_signals_v3, FHRDataset_v6_envelope,TransformerClassifier_step3_twoInputs, create_mask,tversky_loss, tversky_focal_loss
...
input_dim = 2  # For univariate time series, 2 for multivariate
...
patch_sizes =[30, 60, 120, 240, 480] #Assuming time steps per patch [30]  # # 

batch_size = 256
model_name = 'Transformer_BMVClassifier_NewData_FHR+PeakEnv'
...
learning_rate = [0.01,0.001, 0.00001]    #0.00001

print("\n THIS PROGRAM IS UPDATED WITH NORMALIZATION WITH PEAK ENVELOPE with 220 max and min values,\
 \nwith CROSS ENTROPY LOSS function instead of Tversky focal loss, with SAVGOL Envelope.\n")
...
train_loader = DataLoader(train_dataset, batch_size=batch_size, collate_fn=collate_fn_twoInputs, shuffle=True)
val_loader = DataLoader(val_dataset, batch_size=batch_size, collate_fn=collate_fn_twoInputs, shuffle=False)
test_loader = DataLoader(test_dataset, batch_size=batch_size, collate_fn=collate_fn_twoInputs, shuffle=False)
                ...
                for sch in lr_scheduler:
                    for lr in learning_rate:
                        for i in range(iterations):
                            ...
                            model = TransformerClassifier_step3_twoInputs(input_dim, d_model, nhead, num_encoder_layers, dim_feedforward, patch_size,  dropout)
                            ...
                            plot_random_signals_v3(train_loader,  model_name, save_path=path, norm='MinMax', num_signals=2)
                            ...
                            for epoch in range(num_epochs):
                                ...
                                for batch in train_loader:
                                    data, labels_30min, labels_resus = batch
                                    if torch.cuda.is_available():
                                        data = data.to(device)
                                    ...
                                    # Forward pass
                                    outputs_resus= model(data)

                                    # Calculate Tversky loss for resuscitation and outcome
                                    loss_resus = criterion(outputs_resus, labels_resus)
                                ...
                                with torch.no_grad():
                                    for batch in val_loader:
                                        data, labels_30min, labels_resus = batch
                                        if torch.cuda.is_available():
                                            data, labels_30min, labels_resus = data.to(device), labels_30min.to(device), labels_resus.to(device)
                            ...
                            plt.savefig(f"{path}/LossCurve-bmvClassifiers_OneChannel.png")
                            # plt.show()
                            ...
                            data = {"files": file_names, "ground_truth_bmv": label_resus,"predicted_bmv": preds_resus, "prob_bmv": probs_resus}

                            dframe = pd.DataFrame(data)
                            print("Length of Validation data ", len(dframe))
                            with pd.ExcelWriter(f"{path}/Validation_bmvclasses_onechannel.xlsx") as wr:
                                    dframe.to_excel(wr, index=False)

                            # test_acc_30min = 100 * correct_30min / total_30min
                            test_acc_resus = 100 * correct_resus / total_resus

                            print(f'Valid-Accuracy BMV: {test_acc_resus:.2f} %')
                            ...
                            with pd.ExcelWriter(f"{path}/test_bmvclasses_onechannel.xlsx") as wr:
                                    dframe.to_excel(wr, index=False)
