...
# This is transformer-based classification model for RESUS and Outcome classification training. It uses new version of dataset and loads a different version of dataloader.
# this uses ONE input for training. Univariate training.
...
from myfunctions import  collate_fn, convert_batch_list, make_pretty_cm, get_class_weights, weights_to_tensor, plot_random_signals_v2, FHRDataset_v6,\
...
batch_size = 256
model_name = 'Transformer_2classes_NewData_model1_scheduler'
...
train_dataset = FHRDataset_v6(train_dir, sequence_length=sequence_length) # 'zscore', 'minmax'
val_dataset = FHRDataset_v6(val_dir, sequence_length=sequence_length) # new data is already normalized using minmax
test_dataset = FHRDataset_v6(test_dir, sequence_length=sequence_length) # new data is already normalized using minmax
                    ...
                    for lr in learning_rate:
                        for i in range(iterations):
                            ...
                            for epoch in range(num_epochs):
                                ...
                                print(f"Epoch {epoch+1}, Val Loss BMV: {val_loss_resus:.4f}, Val Loss Outcome: {val_loss_30min:.4f}")
                                print(f"Epoch {epoch+1}, Val Acc BMV: {val_acc_resus:.2f}%, Val Acc Outcome: {val_acc_outcome:.2f}%")
                            ...
                            with torch.no_grad():
                                for batch in val_loader:
                                    ...
                                    outputs_resus, outputs_30min = model(data)
                            ...
                            print(f'Test-Accuracy BMV: {test_acc_resus:.2f}%, Test-Accuracy 30min: {test_acc_30min:.2f}%')# 0 not attempted, 1 attempted
